...
- `check.svg`: Utilizado para indicar Materias Aprobadas (`/materias`), como el botón "Marcar como Completada" en una alerta (`/alertas`), y para el estado de las sesiones 'Parcial' en 'Sesiones Completadas Hoy' (`/area-estudio`).
...
- `square-pen.svg`: Icono de edición. Al hacer `:hover` en una tarjeta de Marcador, se ubica en la esquina derecha como el 2do botón de izquierda a derecha (`/area-estudio`). También se usa para editar mazos y tarjetas (`/flashcards`). Botón "Editar Alerta" en la lista de alertas (`/alertas`). Botón "Actualizar" monto de cuota en el panel de administración (`/admin/cuotas`).
